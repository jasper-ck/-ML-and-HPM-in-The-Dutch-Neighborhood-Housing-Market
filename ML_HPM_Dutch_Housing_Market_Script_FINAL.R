...
combined_ds_1617_a <- combined_ds_1617 %>% 
  drop_na(afstand_tot_school_97, afstand_tot_huisartsenpraktijk_94, 
          totaal_diefstal_uit_woning_schuur_ed_106)

summary(combined_ds_1617_a)       

# train/ test split ------------------------------------------------------------
set.seed(123)
train.index1 <- createDataPartition(combined_ds_1617_a$gemiddelde_woningwaarde_35, 
                                    p = 0.7, list = FALSE)
train_combined <- combined_ds_1617_a[ train.index1,]
test_combined  <- combined_ds_1617_a[-train.index1,]
...
# execute pmm ------------------------------------------------------------------  
train_combined_ready <- mice(train_combined [7:95],m=5,maxit=10,meth='pmm',seed=123)
test_combined_ready  <- mice(test_combined[7:95],m=5,maxit=10,meth='pmm',seed=123)
...
# make copy 
combi_ds_tr_tst_copy <- combi_ds_tr_tst
combi_ds_tr_tst_copy <- combi_ds_tr_tst_copy[1:88]

combi_ds_tr_tst_copy$gemiddelde_woningwaarde_35 <- combi_ds_tr_tst$gemiddelde_woningwaarde_35

# HZ Test 
HZ.test(combi_ds_tr_tst_copy, qqplot = FALSE)

# qqplot
qqnorm(combi_ds_tr_tst_copy$gemiddelde_woningwaarde_35, pch = 1, frame = FALSE)
qqline(combi_ds_tr_tst_copy$gemiddelde_woningwaarde_35, col = "steelblue", lwd = 2
...
cor(combi_ds_tr_tst_copy, method = c("spearman"))
corrplot(cor(combi_ds_tr_tst_copy))
...
# KMO scores 
kmo(combi_ds_tr_tst_copy)

# Bartlett's Test
nieuw <- combi_ds_tr_tst_copy  %>% 
  pivot_longer(cols = 4:ncol(combi_ds_tr_tst_copy), names_to = 'group', 
               values_to = 'value')
...
View(train_combined_ready_pca)

# final scree plot
prin_compt <- prcomp(train_combined_ready_pca[1:85], scale. = F)
names(prin_compt)
...
train_combined_ready_MINREM <- train_combined_ready[1:85]
train_combined_ready_MINREM$gemiddelde_woningwaarde_35 <- train_combined_ready$gemiddelde_woningwaarde_35

# compute MINREM 
set.seed(123)
resul.obs.ml <-min.rem_red_demo(base.dat=train_combined_ready_MINREM,prop.entrena=0.7,nrep=100,
                                secu.ini=200,p.fin=.2,delta=100,Ndepura=1100,pvali=0.05,
...
MINREM_model_train <- readRDS(file = "MINREM_model_train.rds")
MINREM_model_test <- readRDS(file = "MINREM_model_test.rds") 

View(MINREM_model_train)
...
r2_socio_envir <- r2_check(environmental_char_test$gemiddelde_woningwaarde_35, 
                           pred_MLR4)
...
# load train/test set ----------------------------------------------------------
pca_train_1 <- readRDS(file = "pca_train.rds")
pca_test_1 <- readRDS(file = "pca_test.rds")
